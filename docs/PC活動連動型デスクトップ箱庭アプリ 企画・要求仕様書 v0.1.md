...
2026-09-11の本人判断に基づき、Windows GUIではPCロック中のSimulation停止をV1前の必須条件とする。解除後は同じセッションを再開する。ただし手動の一時停止・活動取得OFF・非表示・終了済み状態は維持し、停止時間分をまとめて進めない。

ロックとスリープの休止理由は分離し、すべての理由が解除されるまで休止する。停止中の活動を復帰後に遡って反映しない。通知登録または初期状態の確認に失敗した場合は理由を表示して休止し、再試行する。CLIの再現実行にはOS休止連動を適用しない。
...
保存済みの設定・Discoveryを読み込めない場合は、問題のファイルを原位置のまま保護し、そのファイルだけ保存を休止する。正常な設定・Discoveryの利用と保存は継続する。未対応の保存スキーマや項目を勝手に変換・上書きしない。設定を読み込めない場合は活動取得OFFの安全な初期状態とし、本人の選択なしに取得を開始しない。

保存されないデータ種別をユーザーへ説明する。保護中の一時状態を原本へ自動で上書きせず、修復・復元後の再起動で再確認する。初期化・削除は31.7の確認規定に従う。
...
v0.9.7移行方針（2026-09-09本人決定）: 既存GUIユーザーの活動取得ON/OFFは維持し、操作を妨げない説明欄を表示する。説明を閉じてもON/OFFは変えない。旧設定のONを同意の履歴として扱わない。新規GUIでは取得する・取得せず使う・終了を選ぶまで実活動取得元を起動しない。実装判断は[ADR 0009](adr/0009-activity-notice.md)に記録する。
